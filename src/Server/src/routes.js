...
import { controllerDonationSearch, controllerInsertDonation, controllerRemoveDonation, controllerGetAlimentos, controllerGetDonationsByGroup } from './Controllers/donationController.js'
...
//listar alimentos
r.get('/alimentos', async (_, res) => {
    const data = await controllerGetAlimentos()
    res.status(data.status_code).json(data)
})

//listar doações por grupo
r.get('/doacoes/:id', async (req, res) => {
    const data = await controllerGetDonationsByGroup(req.params.id)
    res.status(data.status_code).json(data)
})
export default r
